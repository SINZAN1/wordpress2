 ...
 */
function country_inn_breadcrumb_trail( $args = array() ) {
...
class Breadcrumb_Trail {
	...
	protected function set_labels() {
		...
		$defaults = array(
			...
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'country-inn' ),

			'home'                => esc_html__( 'Home', 'country-inn' ),

			'error_404'           => esc_html__( '404 Not Found', 'country-inn' ),

			'archives'            => esc_html__( 'Archives', 'country-inn' ),

			// Translators: %s is the search query.

			'search'              => esc_html__( 'Search results for: %s', 'country-inn' ),

			// Translators: %s is the page number.

			'paged'               => esc_html__( 'Page %s', 'country-inn' ),

			// Translators: %s is the page number.

			'paged_comments'      => esc_html__( 'Comment Page %s', 'country-inn' ),

			// Translators: Minute archive title. %s is the minute time format.

			'archive_minute'      => esc_html__( 'Minute %s', 'country-inn' ),

			// Translators: Weekly archive title. %s is the week date format.

			'archive_week'        => esc_html__( 'Week %s', 'country-inn' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'country-inn' ) ) );
	...
	protected function add_minute_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'country-inn' ) ) );
	...
	protected function add_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			...
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'country-inn' ) ) );
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'country-inn' ) ) );

		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'country-inn' ) ) );

		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'country-inn' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'country-inn' ) ) );
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' === $tag ) {
					...
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'country-inn' ) ) ) );

				} elseif ( '%monthnum%' === $tag ) { // If using the %monthnum% tag, add a link to the monthly archive.

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'country-inn' ) ) ) );

				} elseif ( '%day%' === $tag ) { // If using the %day% tag, add a link to the daily archive.

					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'country-inn' ) ) ) );
